...
//! URL/path in the host browser/editor (through the flatpak host escape, since the toolbox has no
//! `xdg-utils`), and make an engine-relative path absolute. Stringly errors here (`Result<_, String>`)
...
pub fn open_url_in_browser(url: &str) -> Result<(), String> {
    ...
    spawn_first(candidates, url).map_err(|err| format!("open {url}: {err}"))
}
...
pub fn open_in_vscode(path: &str) -> Result<(), String> {
    ...
    spawn_first(candidates, &target).map_err(|err| format!("open {path} in vs code: {err}"))
}
