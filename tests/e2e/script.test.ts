...
test("a script slot moves its entity during play; stop restores the authored scene", async () => {
  ...
  const status = await engine.call("get-script-status");
  ...
  expect((await engine.call("get-script-status")).instances).toBe(0);
...
test("script input exposes held keys to Lua", async () => {
  ...
  const moved = await engine.call("inspect", { entity: cube.id });
...
test("slots on one entity run in list order within a tick", async () => {
  ...
  await engine.call("play");
  expect((await engine.call("get-script-status")).instances).toBe(2);
  await engine.settle();
...
test("a script error is contained: drained with a traceback, play pauses, the host survives", async () => {
  ...
  expect((await engine.call("get-play-state")).state).toBe("paused");

  const drained = await engine.call("drain-script-errors", { since: 0 });
  ...
  const again = await engine.call("drain-script-errors", {
  ...
  expect((await engine.call("stop")).state).toBe("edit");
  await engine.call("destroy-entity", { entity: cube.id });
});
...
test("sa.log lands in drain-script-logs tagged with the logging entity, and does not pause play", async () => {
  const cursor = (await engine.call("drain-script-logs", { since: 0 })).highWaterSeq;
  const robot = await engine.call("create-entity", { name: "Robot" });
  await attachScripts(engine, robot.id, ["logger.lua"]);

  await engine.call("play");
  await engine.settle();

  const drained = await engine.call("drain-script-logs", { since: cursor });
  ...
  const again = await engine.call("drain-script-logs", { since: drained.highWaterSeq });
  expect(again.events).toEqual([]);
