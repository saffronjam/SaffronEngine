...
test("a down-ray hits the floor; an up-ray misses", async () => {
  ...
  const down = await engine.call("raycast", {
    origin: { x: 5, y: 5, z: 0 },
    dir: { x: 0, y: -1, z: 0 },
    maxDist: 20,
  });
  ...
  const onBox = await engine.call("raycast", {
    origin: { x: 0, y: 5, z: 0 },
    dir: { x: 0, y: -1, z: 0 },
    maxDist: 20,
  });
  ...
  const up = await engine.call("raycast", {
    origin: { x: 0, y: 5, z: 0 },
    dir: { x: 0, y: 1, z: 0 },
    maxDist: 20,
  });
...
test("a sphere-cast grazes the box edge where a thin ray misses it", async () => {
  ...
  const thin = await engine.call("raycast", {
  ...
  const sphere = await engine.call("shapecast", {
    origin: { x: 0.7, y: 5, z: 0 },
    dir: { x: 0, y: -1, z: 0 },
    radius: 0.5,
    maxDist: 20,
...
test("a Lua script can query the world via sa.raycast", async () => {
  ...
  const prober = (await engine.call("create-entity", { name: "Prober" })).id;
  ...
  const t = await engine.call("get-world-transform", { entity: prober });
