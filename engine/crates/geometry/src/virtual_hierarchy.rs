...
/// One (variation, phenotype) combination's active-use mask: which micro-instance uses
/// draw for instances rendered with that combination. Bit `u` of word `u / 32` covers
/// use `u`; the word count always equals `ceil(uses / 32)`.
#[derive(Clone, Debug, PartialEq, Eq)]
...
const fn identity_transform_bits() -> [i32; 16] {
    [
...
fn bounds_for_positions(vertices: &[PortableSourceVertex]) -> Result<PortableBounds> {
    let first = vertices
        .first()
...
fn bounds_for_vertex_indices(mesh: &PortableSourceMesh, indices: &[u32]) -> Result<PortableBounds> {
    let first = *indices
        .first()
...
fn bounds_for_indexed(mesh: &PortableSourceMesh, indices: &[u32]) -> Result<PortableBounds> {
...
fn multiply_unit(first: u16, second: u16) -> u16 {
...
const fn standard_moments() -> PortableMaterialMoments {
    PortableMaterialMoments {
        occupancy: u16::MAX,
...
mod tests {
    ...
    fn source_vertex(position_bits: [i32; 3], uv_bits: [i32; 2]) -> PortableSourceVertex {
    ...
    fn multi_page_input() -> PortableHierarchyInput {
