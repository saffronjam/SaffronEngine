...
//! carrying the device-local cull-list SSBO, the host-mapped params UBO the lighting consumers
//! read, and the cull + composite descriptor sets. The cull list and params UBO are rewritten every
//! frame, so each frame in flight needs its own copy. The linear-**repeat** cascade sampler and the
//! two compute set layouts are shared across slots.
//!
//! `gdf-cull` bins each [`crate::SdfInstance`]'s world AABB into the cascade(s) it touches,
//! atomic-appending its index into a per-cascade compacted list. `gdf-composite` `min()`s the
//! culled bricks' signed distance into each dirty voxel; empty voxels store the conservative open
//! distance (`+max-encode`, never zero) so the sphere-march leaps across empty space. The finest
//! cascade also splats each cell's nearest-occluder base color into the lite albedo cache the DDGI
//! trace reads — a flat per-cell color with no normal, shading, or within-cell material
//! resolution, which is the fidelity cap of the DDGI hit term.
...
/// for `t < GDF_NEAR_HANDOFF` and the Global-SDF beyond, which bounds light leaking.
