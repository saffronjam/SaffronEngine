...
impl VegetationArtifactStore {
    ...
    /// Renews one held claim's lease by bumping the claim file's modification time.
    pub fn heartbeat_work_claim(&self, manifest: ContentHash, index: u32) -> Result<()> {
        let path = self
            .work_claims_dir(manifest)
            .join(format!("{index}.claim"));
