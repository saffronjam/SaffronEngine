...
/// One plant's wind prepass record, read back from the GPU, beside the authored family
/// response that produced it.
///
/// Every raster pass applies these stored words rather than re-evaluating wind, so this
/// is what the plant is actually doing — not a second CPU model of it.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export)]
pub struct VegetationWindRecordResult {
    ...
    /// World-space sway displacement at the previous frame's time — motion vectors read
    /// this, so a mismatch here is a ghosting trail.
    ...
    /// True when the interaction cascade covering this plant changed since the previous
    /// frame. The field re-centres on the camera and zeroes every texel that scrolls in,
    /// so the displacement above jumped rather than moved, and the plant is marked
    /// reactive for the frame instead of being reprojected.
    pub interaction_reset: bool,
    /// The branch mode's `sin`/`cos` at the current time then the previous time. The
    /// vertex path applies a per-use phase offset through this quadrature, which is how
    /// one stored record moves every assembly part with its own phase.
