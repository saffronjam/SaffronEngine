...
//!
//! Interchange brings instanced points in as authored anchors and writes them back out through the
//! canonical point schema and nothing else: an imported point becomes an [`ExplicitPlantAnchor`]
//! like any hand-placed plant, or it is rejected. Two rules make a round trip safe. A source
//! attribute this vocabulary cannot express is **reported**, never silently kept — a retained blob
//! would be a second truth the runtime cannot read. And an instance's stable identity survives, so
//! re-importing an updated scatter re-addresses the same plants and authored overrides keyed to
...
use crate::{
    Error, ExplicitPlantAnchor, InteractionPolicy, PlantDimensions, PlantFlags, PlantId,
    PlantLifecycle, PlantPoint, Result,
};

/// Instances one interchange payload may carry.
...
pub struct PointInterchange {
    ...
    /// Source attributes this vocabulary cannot express, in canonical order.
...
/// The first `N` lanes of a source tuple, padded with `fallback`.
pub(crate) fn lanes<const N: usize>(values: &[f64], fallback: f64) -> [f64; N] {
    std::array::from_fn(|lane| values.get(lane).copied().unwrap_or(fallback))
}

/// The prototype one source name resolves to.
///
/// # Errors
///
/// [`Error::ArtifactFormat`] under `field` when the name is bound to no plant family.
pub(crate) fn resolve_prototype(
    format: &'static str,
    field: &str,
    name: String,
    families: &BTreeMap<String, Uuid>,
) -> Result<PointPrototype> {
    let family = *families.get(&name).ok_or_else(|| Error::ArtifactFormat {
        format,
        field: field.to_owned(),
    })?;
    Ok(PointPrototype { name, family })
}
...
/// same plant every time it arrives.
///
/// # Errors
///
...
pub fn interchange_to_anchors(
    ...
    layer: u128,
    dimensions: &BTreeMap<u64, PlantDimensions>,
    ...
    for instance in payload.active() {
        ...
        let orientation = quantize_orientation(instance.orientation)
            .ok_or_else(|| field("instances.orientation"))?;
        ...
        anchors.push(ExplicitPlantAnchor {
            ...
            point: PlantPoint {
                ...
                // An imported point is authored truth, not a sampler draw, so nothing else can
                // reproduce it and the identity is the deterministic key.
...
/// `name` or `variant` (the prototype). Every other attribute is reported as unsupported.
///
/// # Errors
///
...
pub fn read_houdini_points(
    ...
    for (index, point) in positions.iter().take(count).enumerate() {
        ...
        let prototype = match index_of.get(&name) {
            ...
            None => {
                let resolved =
                    resolve_prototype("houdini geo", "name.family", name.clone(), families)?;
                let slot = u32::try_from(prototypes.len()).map_err(|_| field("prototypes"))?;
                prototypes.push(resolved);
                index_of.insert(name, slot);
                slot
            }
        };
        let column = |name: &str| -> Option<&Vec<f64>> {
            columns.get(name).and_then(|values| values.get(index))
        };
        let orientation = column("orient")
            .or_else(|| column("rot"))
            .map_or([0.0, 0.0, 0.0, 1.0], |tuple| lanes(tuple, 0.0));
        ...
        let uniform = column("pscale")
            .and_then(|tuple| tuple.first())
            .copied()
            .unwrap_or(1.0);
        let scale = column("scale")
            .map_or([1.0; 3], |tuple| lanes(tuple, 1.0))
            .map(|value| value * uniform);
        instances.push(PointInstance {
            prototype,
            position: lanes(point, 0.0),
            orientation,
            scale,
...
pub fn write_houdini_points(payload: &PointInterchange) -> JsonValue {
    ...
    let per_instance = |lane: fn(&PointInstance) -> JsonValue| -> Vec<JsonValue> {
        payload.instances.iter().map(lane).collect()
    };
    let point_attributes = json!([
        numeric("P", 3, per_instance(|instance| json!(instance.position))),
        numeric(
            "orient",
            4,
            per_instance(|instance| json!(instance.orientation))
        ),
        numeric("scale", 3, per_instance(|instance| json!(instance.scale))),
        numeric(
            "id",
            1,
            per_instance(|instance| json!([instance.stable_id]))
        ),
        strings(
            "name",
            payload
                ...
                .collect(),
            per_instance(|instance| json!(instance.prototype))
        ),
    ]);
    json!([
        "fileversion",
...
pub fn gltf_instancing_to_interchange(
    ...
    for set in &instancing.sets {
        let resolved =
            resolve_prototype("gltf instancing", "node.family", set.name.clone(), families)?;
        ...
        for instance in &set.instances {
            instances.push(PointInstance {
                ...
                stable_id: instance.id.unwrap_or(instances.len() as u64),
                active: true,
            });
        }
    }
...
/// A family's dimensions at a nominal height, shared by the interchange test modules.
#[cfg(test)]
pub(crate) fn test_dimensions(height: f64, crown: f64) -> PlantDimensions {
    let scalar = |value: f64| DecisionScalar::from_f64(value).expect("finite scalar");
    PlantDimensions {
        height: scalar(height),
        trunk_radius: scalar(crown / 10.0),
        crown_radius: [scalar(crown); 2],
        root_radius: [scalar(1.0); 2],
        local_bounds_min: [scalar(-crown), scalar(0.0), scalar(-crown)],
        local_bounds_max: [scalar(crown), scalar(height), scalar(crown)],
    }
}
...
mod tests {
    ...
    fn dimensions() -> BTreeMap<u64, PlantDimensions> {
        BTreeMap::from([(77, test_dimensions(6.0, 2.0))])
    }
    ...
    fn an_instance_becomes_an_authored_anchor() {
        ...
        assert_eq!(anchor.point.flags, PlantFlags::AUTHORED);
