...
- [x] Extend `engine/crates/rendering/src/render_graph.rs` with imported and graph-owned persistent/
...
- [x] Derive compute-write → indirect-read, compute-write → vertex/index/BDA-read, transfer, queue
...
- [x] stable generational handles and global vertex/index/cluster/part/voxel/page arenas;
- [x] BDA vertex pulling or global arena offsets for portable indexed draws;
- [x] global immutable prototype, geometry, material, texture, coverage, skeleton, and page tables;
- [x] per-draw records containing geometry, material, instance, deformation, representation, and
  cluster state, addressed by `drawID`/`firstInstance`/bin base rather than descriptor rebinding;
- [x] a small fixed PSO-bin vocabulary for representation, coverage, sidedness, surface model,
...
- [x] optimized triangle clusters with portable vertex/primitive limits, quantized local positions,
...
- [x] hierarchy partitioning and simplification with watertight/solid rules for contiguous geometry;
- [x] plant assembly tables retaining repeated semantic parts as micro-instance transforms rather
  than expanding each branch/leaf;
- [x] aggregate voxel-brick clusters for disconnected foliage, storing occupancy and the coverage/
  material/normal moments above;
- [x] an appearance error including silhouette, projected coverage, transmitted energy, material
  variation, and normal-distribution error—not geometry distance alone;
- [x] hierarchy cuts that may mix triangle and voxel nodes and guarantee a drawable coarse root;
- [x] page dependencies, parent-before-child order, conservative static/deformed bounds, and
  representation transition metadata; and
- [x] portable voxel output as cooked indexed surfaces or compute expansion, so MoltenVK renders the
